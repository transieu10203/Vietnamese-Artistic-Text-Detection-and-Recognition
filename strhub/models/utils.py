...
def get_pretrained_weights(experiment):
    ...
    X["head.weight"]=torch.cat((X["head.weight"],torch.randn(134,384)))
    X["head.bias"]=torch.cat((X["head.bias"],torch.randn(134)))
    X['text_embed.embedding.weight']=torch.cat((X['text_embed.embedding.weight'],torch.randn(134,384)))
    return X
